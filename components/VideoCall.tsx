    ...
    return (
        <div className="h-screen flex flex-col items-center bg-gray-900 text-white p-4">
            ...
            <div className="flex w-full max-w-4xl h-[70vh] border-2 border-gray-700 rounded-lg overflow-hidden">
                <video ref={localVideoRef} autoPlay playsInline muted className="w-1/2 h-full bg-black"></video>
                <video ref={remoteVideoRef} autoPlay playsInline className="w-1/2 h-full bg-black"></video>
            </div>

            {!isConnected && <p className="mt-4 text-yellow-300">Waiting for another user...</p>}

            <div className="mt-4 flex space-x-4">
                <button onClick={startCall} className="px-4 py-2 bg-blue-500 hover:bg-blue-700 rounded">
                    Start Call
                </button>
                <button onClick={toggleCamera} className="px-4 py-2 bg-green-500 hover:bg-green-700 rounded">
                    {isCameraOn ? "Turn Camera Off" : "Turn Camera On"}
                </button>
                <button onClick={toggleMicrophone} className="px-4 py-2 bg-red-500 hover:bg-red-700 rounded">
